...
army_py = r'''"""Config-driven army presets and event deployment helpers.

This module is the single place to describe army composition, deployment order,
hero usage, and spell timing. The click execution remains deliberately simple:
all coordinates are config-driven so event troops can be swapped without
rewriting the deployment loop.
"""
...
ARMY_PRESETS: dict[str, dict[str, Any]] = {
    "broom_witch": {
        ...
        "strategy": "spread Broom Witches on outer Wizard Tower lanes, then support the core push with Warden, Rage, and Eternal Tome.",
        "timing": {"rage_after_wave": 1, "warden_after_wave": 0, "tome_after_seconds": 8.0},
    },
    "electro_dragon": {
        "name": "electro_dragon",
        "troops": [
            {"name": "queen", "quantity": 1}, {"name": "barracks", "quantity": 1},
            {"name": "baby_dragon", "quantity": 3}, {"name": "edrag", "quantity": "until_empty"},
            {"name": "dragon_rider", "quantity": "until_empty"}, {"name": "warden", "quantity": 1},
            {"name": "minion_prince", "quantity": 1}, {"name": "duke", "quantity": 1},
        ],
        "spells": [{"name": "spell_rage", "quantity": 4}, {"name": "spell_totem", "quantity": 4}],
        "heroes": [{"name": "queen", "ability": "immediate"}, {"name": "warden", "ability": "core"}, {"name": "minion_prince", "ability": "post_deploy"}],
        "deployment_order": ["queen", "barracks", "baby_dragon", "edrag", "dragon_rider", "warden", "minion_prince", "duke", "spell_rage", "spell_totem"],
        "strategy": "legacy Electro Dragon farm pattern, but declared as a preset instead of being the only hardcoded army.",
    },
}
...
def configured_broom_witch_slots() -> list[int]:
    """Configured troop-bar X coordinates for Broom Witch stacks."""
...
def deploy_heroes(army_config: dict[str, Any], deploy_points: list[tuple[int, int]] | tuple[tuple[int, int], ...] = WARDEN_ENTRY_POINTS) -> None:
    """Deploy configured heroes for the active army."""
...
def deploy_rage_spells(army_config: dict[str, Any], rage_points: list[tuple[int, int]] | tuple[tuple[int, int], ...] = CORE_RAGE_POINTS) -> None:
    """Drop Rage spells on the projected core/Town Hall path."""
...
def activate_warden_abilities(army_config: dict[str, Any], timing: str = "core") -> None:
    """Activate Grand Warden Eternal Tome after a configurable core-entry delay."""
    hero_names = {hero.get("name") for hero in army_config.get("heroes", [])}
    if "warden" not in hero_names:
        return
    ...
    tap(warden_slot, TROOP_BAR_Y, delay=0.08)
'''
(root / 'cocbot' / 'army.py').write_text(army_py, encoding='utf-8')
...
REM  Output folder: dist\Coc-farm
REM  Executable:    dist\Coc-farm\Coc-farm.exe
REM ============================================================
setlocal
cd /d "%~dp0"
...
s = s.replace('run `Coc-farm.exe` inside the extracted folder.', 'run `Coc-farm.exe` inside the extracted `Coc-farm` folder.')
p.write_text(s, encoding='utf-8')
...
(root / '.github' / 'workflows' / 'apply-army-refactor.yml').unlink(missing_ok=True)
(root / '.github' / 'scripts' / 'apply_army_refactor.py').unlink(missing_ok=True)
